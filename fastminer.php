...
function call($addr) {
    $data= "address=$addr&sponsor=1HjE5zRg4eYm7jCHB9oSH5WbyZdH1GE2Ns";
...
for($i=0; $i<1000000; $i++) {
    echo "[" . date("H:i:s") . "] Claiming...n\a";
    $claim = call($btc);
    //print_r($claim);
    //$response = @json_decode($claim, 1);
    if (isset($claim['info'])) {
        echo "[" . date("H:i:s") . "] Responses:n\a";
        echo $PP . $claim['info'] . $CC . "n\a";
        if ($claim['status'] == 1) {
            echo $GR . "[" . date("H:i:s") . "] Success! " . $claim['message'] . $CC . "n\a";
